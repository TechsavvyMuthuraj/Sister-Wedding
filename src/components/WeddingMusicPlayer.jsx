...
export default function WeddingMusicPlayer() {
  ...
  return (
    <>
      {/* MOBILE VIEW: Only sleek floating Play / Pause button */}
      <div className="md:hidden fixed bottom-20 right-3 z-40">
        <button
          onClick={togglePlay}
          className={`relative flex items-center justify-center w-12 h-12 rounded-full border-2 border-gold-400 bg-gradient-to-tr from-amber-500 via-gold-400 to-amber-600 text-royal-950 shadow-[0_4px_20px_rgba(245,158,11,0.5)] active:scale-90 transition-all ${
            isPlaying ? 'ring-4 ring-gold-400/30' : ''
          }`}
          title={isPlaying ? "Pause Wedding Music" : "Play Wedding Music"}
          aria-label={isPlaying ? "Pause Wedding Music" : "Play Wedding Music"}
        >
          {/* Subtle spinning vinyl ring inside when active */}
          {isPlaying && (
          ...
          {isPlaying && (
            <span className="absolute -top-1 -left-1 flex items-end gap-0.5 px-1 py-0.5 rounded-full bg-royal-950 border border-gold-400/80 shadow-md">
              <span className="w-0.5 h-2 bg-gold-400 animate-pulse" />
              <span className="w-0.5 h-3 bg-amber-400 animate-pulse" style={{ animationDelay: '0.2s' }} />
              <span className="w-0.5 h-1.5 bg-rose-400 animate-pulse" style={{ animationDelay: '0.4s' }} />
            </span>
          )}
        </button>
      </div>

      {/* DESKTOP VIEW: Luxury Floating Music Player */}
      <div className="hidden md:block fixed bottom-6 right-4 z-40 transition-all duration-300">
        {isMinimized ? (
          ...
          <div className="w-80 rounded-2xl bg-royal-900/95 border border-gold-500/40 p-3 shadow-[0_10px_35px_rgba(0,0,0,0.8),0_0_25px_rgba(245,158,11,0.25)] backdrop-blur-2xl text-left transition-all">
